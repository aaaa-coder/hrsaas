...
export default {
  ...
  data() {
    const valideMobile = (rule, value, callback) => {
      isValidMobile(value) ? callback() : callback(new Error('手机号必须是11位'))
    }
    const validePassword = (rule, value, callback) => {
      isValidPassword(value) ? callback() : callback('密码必须在6-16位之间哦')
    }
    return {
      loginForm: {
        mobile: '13800000002',
      ...
      loginRules: {
        mobile: [
          { required: true, trigger: 'blur', message: '手机号不能为空哦' },
          { trigger: 'blur', validator: valideMobile }
        ],
        password: [
          { required: true, trigger: 'blur', message: '密码不能为空哦' },
          { trigger: 'blur', validator: validePassword }
        ]
      },
      loading: false,
